...
class AutowareBenchmarkController(Node):
    ...
    def state_callback(self, msg):
        ...
        if was_driving_before and not is_driving_now:
            ...
            # 목표 도달 시점 기록만 (CSV 저장은 run_single_trial에서)
            print(f"*** GOAL REACHED - Signaling trial completion (CSV save will be done in run_single_trial) ***")
        else:
            # 조건이 맞지 않는 경우 디버깅
    ...
    def run_single_trial(self, trial_num):
        ...
        for attempt in range(max_recovery_attempts):
            try:
                ...
                self.driving_start_time = 0.0  # engage 성공 시점 (실제 주행 시작)
                self.actual_driving_time = 0.0
                ...
                self.get_logger().info(f"TRIAL #{trial_num}: DRIVING START TIME: timestamp={self.driving_start_time}, readable={engage_timestamp}")
                
                print(f"*** DRIVING START TIME RECORDED: {self.driving_start_time} at {engage_timestamp} ***")
                ...
                while remaining_time > 0:
                    ...
                    if self.goal_reached.wait(timeout=wait_time):
                        ...
                        self.get_logger().info(f"TRIAL #{trial_num}: TEST END TIME: timestamp={test_end_time}, readable={success_timestamp}")
                        
                        print(f"*** TEST COMPLETED SUCCESSFULLY: end_time={test_end_time} at {success_timestamp} ***")
                ...
                # Timeout occurred
                # 테스트 종료 시간 기록 (타임아웃 시점)
                test_end_time = time.time()
                timeout_timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                
                error_msg = f"TRIAL #{trial_num}: TIMEOUT ({self.max_trial_time} seconds)"
                self.get_logger().error(error_msg)
                self.get_logger().error(f"TRIAL #{trial_num}: TEST END TIME (timeout): timestamp={test_end_time}, readable={timeout_timestamp}")
                
                error_msg = f"TRIAL #{trial_num}: Current state at timeout: {self.current_state}, is_driving: {self.is_driving}"
                self.get_logger().error(error_msg)
                
                print(f"*** TEST TIMEOUT: end_time={test_end_time} at {timeout_timestamp} ***")
                
                # 최종 시간 계산 및 CSV 저장 (타임아웃 실패한 경우)
    ...
    def calculate_and_save_trial_result(self, trial_num, success, test_end_time):
        """간단하고 확실한 시간 계산 및 CSV 저장 함수"""
        try:
            # 간단한 시간 계산 - 종료시간은 항상 동일!
            test_duration = test_end_time - self.trial_start_time if hasattr(self, 'trial_start_time') else 0.0
            
            # 실제 주행 시간 = 테스트 종료 시점 - engage 성공 시점
            if self.driving_start_time > 0:
                self.actual_driving_time = test_end_time - self.driving_start_time
                driving_time_msg = f"TRIAL #{trial_num}: Actual driving time: {self.actual_driving_time:.2f} seconds (engage→end)"
                self.get_logger().info(driving_time_msg)
                print(f"*** CALCULATED DRIVING TIME: {self.actual_driving_time:.2f} seconds (engage→end) ***")
            else:
                # engage 실패한 경우 - 이런 경우는 거의 없지만 안전장치
                self.actual_driving_time = 0.0
                self.get_logger().warn(f"TRIAL #{trial_num}: No driving start time recorded - driving time set to 0")
                print(f"*** WARNING: No driving start time recorded ***")
            ...
            print(f"*** {status_text} - Test: {test_duration:.2f}s, Driving: {self.actual_driving_time:.2f}s ***")
            ...
            print(f"*** ERROR IN CALCULATE_AND_SAVE: {e} ***")
    ...
    def run_benchmark(self):
        ...
        for trial_num in range(1, self.total_trials + 1):
            ...
            # 성공한 경우만 results 리스트에 추가 (통계용, CSV는 이미 calculate_and_save_trial_result에서 저장됨)
            if success:
                result = {
                    'trial': trial_num,
