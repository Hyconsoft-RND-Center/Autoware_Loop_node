...
class AutowareBenchmarkController(Node):
    ...
    def state_callback(self, msg):
        ...
        if prev_state != self.current_state:
            ...
            # 주행 시작 시점 기록 (engage 성공 후가 아닌 상태 변화 시점에서)
            if not was_driving_before and is_driving_now and not self.autonomous_start_time_by_state:
                self.autonomous_start_time_by_state = time.time()
                start_timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                print(f"*** DRIVING START DETECTED BY STATE CHANGE: {self.autonomous_start_time_by_state} at {start_timestamp} ***")
                self.get_logger().info(f"DRIVING START BY STATE: timestamp={self.autonomous_start_time_by_state}, readable={start_timestamp}")
            
            # 주행 종료 시점 기록 (상태 변화 시점에서)
            if was_driving_before and not is_driving_now:
                self.autonomous_end_time_by_state = time.time()
                end_timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                print(f"*** DRIVING END DETECTED BY STATE CHANGE: {self.autonomous_end_time_by_state} at {end_timestamp} ***")
                self.get_logger().info(f"DRIVING END BY STATE: timestamp={self.autonomous_end_time_by_state}, readable={end_timestamp}")
                
                # 상태 변화 기반 주행 시간 즉시 계산
                if self.autonomous_start_time_by_state:
                    driving_duration = self.autonomous_end_time_by_state - self.autonomous_start_time_by_state
                    print(f"*** CALCULATED DRIVING DURATION BY STATE: {driving_duration:.3f} seconds ***")
                    self.get_logger().info(f"DRIVING DURATION BY STATE: {driving_duration:.3f} seconds")
        ...
        if was_driving_before and not is_driving_now and not getattr(self, 'trial_already_saved', False):
            ...
            try:
                ...
                # state_callback에서 실제 주행 시간 계산 - 상태 변화 기반 우선
                actual_driving_time = 0.0
                if self.autonomous_start_time_by_state and self.autonomous_end_time_by_state:
                    actual_driving_time = self.autonomous_end_time_by_state - self.autonomous_start_time_by_state
                    print(f"*** CALCULATED DRIVING TIME (by state): {actual_driving_time:.2f} seconds ***")
                elif self.autonomous_start_time:
                    actual_driving_time = end_time - self.autonomous_start_time
                    print(f"*** CALCULATED DRIVING TIME (by engage): {actual_driving_time:.2f} seconds ***")
                else:
                    print(f"*** WARNING: Could not calculate driving time in state_callback ***")
                    print(f"*** DEBUG: start_by_state={self.autonomous_start_time_by_state}, end_by_state={self.autonomous_end_time_by_state}, start_by_engage={self.autonomous_start_time} ***")
                
                result = {
                    'trial': trial_num,
                    'duration': trial_duration,
                    'actual_driving_time': actual_driving_time,
    ...
    def run_single_trial(self, trial_num):
        ...
        for attempt in range(max_recovery_attempts):
            try:
                ...
                # 상태 변화 기반 시간 기록 변수 초기화
                self.autonomous_start_time_by_state = None
                self.autonomous_end_time_by_state = None
                ...
                self.autonomous_end_time = None  # 종료 시간 초기화
                engage_timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                self.get_logger().info(f"TRIAL #{trial_num}: Test marked as IN PROGRESS - state monitoring active")
                self.get_logger().info(f"TRIAL #{trial_num}: Autonomous driving started at {engage_timestamp}")
                self.get_logger().info(f"TRIAL #{trial_num}: ENGAGE START TIME: timestamp={self.autonomous_start_time}, readable={engage_timestamp}")
                
                # 디버그: 현재까지의 모든 시간 변수 상태 출력
                print(f"*** ALL TIME VARIABLES AFTER ENGAGE ***")
                print(f"autonomous_start_time (engage): {self.autonomous_start_time}")
                print(f"autonomous_start_time_by_state: {getattr(self, 'autonomous_start_time_by_state', 'None')}")
                print(f"autonomous_end_time_by_state: {getattr(self, 'autonomous_end_time_by_state', 'None')}")
                print(f"*** END TIME VARIABLES ***")
                ...
                while remaining_time > 0:
                    ...
                    if self.goal_reached.wait(timeout=wait_time):
                        ...
                        self.get_logger().info(f"TRIAL #{trial_num}: SUCCESS END TIME: timestamp={self.autonomous_end_time}, readable={success_timestamp}")
                        
                        # 디버그: 모든 시간 변수 출력
                        print(f"*** ALL TIME VARIABLES AT SUCCESS ***")
                        print(f"autonomous_start_time (engage): {self.autonomous_start_time}")
                        print(f"autonomous_end_time (success): {self.autonomous_end_time}")
                        print(f"autonomous_start_time_by_state: {getattr(self, 'autonomous_start_time_by_state', 'None')}")
                        print(f"autonomous_end_time_by_state: {getattr(self, 'autonomous_end_time_by_state', 'None')}")
                        print(f"*** END TIME VARIABLES ***")
                        
                        # 실제 주행시간 계산 - 상태 변화 기반 우선
                        if self.autonomous_start_time_by_state and self.autonomous_end_time_by_state:
                            self.actual_driving_time = self.autonomous_end_time_by_state - self.autonomous_start_time_by_state
                            driving_time_msg = f"TRIAL #{trial_num}: Actual driving time (by state): {self.actual_driving_time:.2f} seconds"
                            self.get_logger().info(driving_time_msg)
                        elif self.autonomous_start_time and self.autonomous_end_time:
                            self.actual_driving_time = self.autonomous_end_time - self.autonomous_start_time
                            driving_time_msg = f"TRIAL #{trial_num}: Actual driving time (by engage): {self.actual_driving_time:.2f} seconds"
                            self.get_logger().info(driving_time_msg)
                        else:
                            self.actual_driving_time = 0.0
                            self.get_logger().warn(f"TRIAL #{trial_num}: Could not calculate driving time - start_by_state: {self.autonomous_start_time_by_state}, end_by_state: {self.autonomous_end_time_by_state}, start_by_engage: {self.autonomous_start_time}, end_by_engage: {self.autonomous_end_time}")
                ...
                self.get_logger().error(f"TRIAL #{trial_num}: TIMEOUT END TIME: timestamp={self.autonomous_end_time}, readable={timeout_timestamp}")
                
                error_msg = f"TRIAL #{trial_num}: Current state at timeout: {self.current_state}, is_driving: {self.is_driving}"
                self.get_logger().error(error_msg)
                
                # 디버그: 모든 시간 변수 출력
                print(f"*** ALL TIME VARIABLES AT TIMEOUT ***")
                print(f"autonomous_start_time (engage): {self.autonomous_start_time}")
                print(f"autonomous_end_time (timeout): {self.autonomous_end_time}")
                print(f"autonomous_start_time_by_state: {getattr(self, 'autonomous_start_time_by_state', 'None')}")
                print(f"autonomous_end_time_by_state: {getattr(self, 'autonomous_end_time_by_state', 'None')}")
                print(f"*** END TIME VARIABLES ***")
                
                # 실제 주행시간 계산 (실패한 경우에도)
                if self.autonomous_start_time and self.autonomous_end_time:
                    self.actual_driving_time = self.autonomous_end_time - self.autonomous_start_time
                    driving_time_msg = f"TRIAL #{trial_num}: Actual driving time (failed): {self.actual_driving_time:.2f} seconds"
                    self.get_logger().info(driving_time_msg)
                else:
                    self.actual_driving_time = 0.0
                    self.get_logger().warn(f"TRIAL #{trial_num}: Could not calculate driving time (timeout) - start_time: {self.autonomous_start_time}, end_time: {self.autonomous_end_time}")
